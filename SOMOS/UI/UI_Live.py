...
def run_coverage(output_dir: Path, status: SharedStatus, seed: int, n_oht: int, horizon: float) -> None:
    ...
    try:
        ...
        runner.oht_config.oht_coverage_positioning = True
        runner.oht_config.oht_coverage_window = 900.0
        runner.oht_config.oht_coverage_prior_weight = 1.0
        runner.oht_config.oht_coverage_grid = 0.0
        runner.oht_config.oht_deadlock_recovery = True
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.no_run:
        threading.Thread(
            target=run_coverage,
            args=(output_dir, status, args.seed, args.n_oht, args.horizon),
            daemon=True,
        ).start()
    else:
