...
class OHTVehicle(sim.Component):
    ...
    def _force_proceed_active(self):
        """Whether the force-proceed token granted by deadlock recovery is valid (short time window, auto-expire)."""
        return self.env.now() < getattr(self, '_force_proceed_until', 0.0)
    ...
    def move_one_hop(self, next_node_name):
        ...
        # ★★ deadlock recovery force-proceed: if the next node is physically empty,
        #   ignore front-chain (merge station occupancy beyond the curve) / merge-yield conservatism and advance one hop.
        #   → clears its own station to relieve the queue behind (the actual action that breaks the distributed merge-yield cycle).
        if self._force_proceed_active() and not self._is_next_node_blocked_now(next_node_name):
            self._reserve_zcu([next_node_name])
            yield from _do_move()
            return
        ...
        if merge_station is not None:
            # ★ deadlock recovery: a leader granted force-proceed ignores merge-yield and enters
            #   (breaks the global wait-for cycle). Physical occupancy (STEP0) / ZCU already passed.
            if self._force_proceed_active():
        ...
        if upcoming_merge is not None and not self._force_proceed_active():
...
class _DeadlockRecoveryMonitor(sim.Component):
    """Periodically call runner._deadlock_recovery() to detect and recover merge-yield cycles."""
    def setup(self, runner):
        self.runner = runner

    def process(self):
        dt = float(getattr(self.runner.oht_config, 'oht_dl_recovery_dt', 8.0))
        while True:
            yield self.hold(dt)
            try:
                self.runner._deadlock_recovery()
            except Exception:
                pass


class _CoveragePositioningMonitor(sim.Component):
    """(A) Periodically call dispatcher._assign_coverage_positioning() → match the spatial distribution of idle OHTs
    to the gantt demand density (targeting the source_wait tail). Runs on top of recovery."""
    def setup(self, runner):
        self.runner = runner

    def process(self):
        cfg = self.runner.oht_config
        dt = float(getattr(cfg, 'oht_coverage_dt', 45.0))
        win = float(getattr(cfg, 'oht_coverage_window', 900.0))
        grid = float(getattr(cfg, 'oht_coverage_grid', 0.0))
        pw = float(getattr(cfg, 'oht_coverage_prior_weight', 1.0))
        dm = getattr(self.runner, '_oht_dm', None)
        self.runner._cov_calls = 0
        self.runner._cov_assigns = 0
        while True:
            yield self.hold(dt)
            if dm is None:
                continue
            self.runner._cov_calls += 1
            try:
                n = dm._assign_coverage_positioning(self.env.now(), window=win,
                                                    grid=grid, prior_weight=pw)
                self.runner._cov_assigns += int(n or 0)
            except Exception:
                self.runner._cov_errs = getattr(self.runner, '_cov_errs', 0) + 1
...
class SimulationRunner:
    ...
    def _deadlock_recovery(self):
        """★ Global deadlock detection + recovery. When merge-waiting exceeds the threshold time, build a global wait-for graph
        (queue = waiting on the forced_next occupant, leader = merge-yield target) to find cycles,
        and give the min-vid leader of each cycle a force-proceed token to ignore yield and enter, breaking the loop."""
        if not getattr(self.oht_config, 'oht_deadlock_recovery', False):
            return 0
        now = self.env.now()
        thresh = float(getattr(self.oht_config, 'oht_dl_merge_wait_thresh', 15.0))
        if not any(getattr(v, 'merge_wait_since', None) is not None
                   and (now - float(v.merge_wait_since)) > thresh for v in self.vehicles):
            return 0
        occ = {v.pos_node: v for v in self.vehicles}
        waitfor, is_leader = {}, {}
        for v in self.vehicles:
            nxt = v._forced_next_from(v, v.pos_node)
            tgts, leader = set(), False
            if nxt is not None:
                w = occ.get(nxt)
                if w is not None and w is not v:
                    tgts.add(w)                         # queue: waiting on the front (occupying) vehicle
                else:
                    ms, chain = v._curve_merge_station(nxt)
                    if ms is not None:
                        leader = True                   # forced_next empty but before the merge → leader
                        preds = set(v._incoming_predecessors(ms)) - set(chain) - {v.pos_node}
                        for nd in list(preds) + [ms]:
                            o = occ.get(nd)
                            if o is not None and o is not v:
                                tgts.add(o)
            waitfor[v], is_leader[v] = tgts, leader
        ...
        def find_cycle(edges):
            ...
            def dfs(u, stk):
                ...
                for w in edges.get(u, ()):
                    if color.get(w, 0) == 1:
                        return stk[stk.index(w):]
                    if color.get(w, 0) == 0:
                        c = dfs(w, stk)
                        if c:
                            return c
                stk.pop(); color[u] = 2; return None
            for u in edges:
                if color.get(u, 0) == 0:
                    c = dfs(u, [])
                    if c:
        ...
        edges = {v: set(t) for v, t in waitfor.items()}
        window = 2.0 * float(getattr(self.oht_config, 'oht_dispatch_dt', 2.0))
        granted = 0
        while granted < 30:
            cyc = find_cycle(edges)
            if not cyc:
                break
            cands = [x for x in cyc if is_leader.get(x)] or list(cyc)
            if getattr(self.oht_config, 'oht_dl_gantt_priority', False):
                # gantt-informed: pass first the lot most behind plan (lateness=now-planned_end)
                def _lateness(x):
                    t = getattr(x, 'assigned_task', None)
                    return (now - float(getattr(t, 'planned_end', now))) if t is not None else -1e18
                pick = max(cands, key=_lateness)
            else:
                pick = min(cands, key=lambda x: getattr(x, 'vid', 1 << 30))
            pick._force_proceed_until = now + window
            edges[pick] = set()                          # remove this leader's wait → the loop is broken
            granted += 1
        self._dl_grants = getattr(self, '_dl_grants', 0) + granted   # cumulative intervention count (for logging)
        return granted
    ...
    def build_logic(self):
        ...
        self._oht_dm = OHTDecisionMaker(
            ...
            predispatch_lookahead=getattr(
                self.oht_config, 'oht_predispatch_lookahead', 0.0),
            congestion_node_penalty=getattr(
                self.oht_config, 'oht_congestion_node_penalty', 8.0),
            congestion_edge_penalty=getattr(
                self.oht_config, 'oht_congestion_edge_penalty', 12.0),
            buffer_block_penalty=getattr(
                self.oht_config, 'oht_buffer_block_penalty', 30.0),
            predictive_late_weight=getattr(
                self.oht_config, 'oht_predictive_late_weight', 0.25),
            predictive_wait_weight=getattr(
                self.oht_config, 'oht_predictive_wait_weight', 0.50),
            blocking_weight=getattr(self.oht_config, 'oht_blocking_weight', 0.0),
            load_sec=float(getattr(self.oht_config, 'load_time_min', 10.0)),
            unload_sec=float(getattr(self.oht_config, 'unload_time_min', 10.0)),
            urgency_weight=getattr(self.oht_config, 'oht_urgency_weight', 0.0),
            urgency_late_weight=getattr(self.oht_config, 'oht_urgency_late_weight', 1.0),
            urgency_starve_weight=getattr(self.oht_config, 'oht_urgency_starve_weight', 150.0),
            urgency_starve_ref=getattr(self.oht_config, 'oht_urgency_starve_ref', 2.0),
            urgency_cap_ratio=getattr(self.oht_config, 'oht_urgency_cap_ratio', 0.6),
            idle_positioning=getattr(self.oht_config, 'oht_idle_positioning', False),
            positioning_lookahead=getattr(self.oht_config, 'oht_positioning_lookahead', 90.0),
            congestion_cost_weight=getattr(self.oht_config, 'oht_congestion_cost_weight', 0.0),
            plan_bottleneck_boost=getattr(self.oht_config, 'oht_bottleneck_boost', 0.0),
            plan_bottleneck_types=getattr(self.oht_config, 'oht_bottleneck_types', 'PHOTO'),
            plan_pair_yield=getattr(self.oht_config, 'oht_pair_yield', False),
            plan_pair_yield_factor=getattr(self.oht_config, 'oht_pair_yield_factor', 1.5),
            plan_lookahead_positioning=getattr(self.oht_config, 'oht_plan_lookahead', False),
            plan_congestion_weight=getattr(self.oht_config, 'oht_plan_congestion_weight', 0.0),
            plan_bucket_sec=getattr(self.oht_config, 'oht_plan_bucket_sec', 90.0),
            plan_tasks_csv=getattr(self.oht_config, 'oht_plan_tasks_csv', ''),
            plan_congestion_cap_ratio=getattr(self.oht_config, 'oht_plan_congestion_cap_ratio', 1.0),
            plan_starve_guard_sec=getattr(self.oht_config, 'oht_plan_starve_guard_sec', 300.0),
            plan_time_anchor=getattr(self.oht_config, 'oht_plan_time_anchor', True),
            plan_adaptive_w=getattr(self.oht_config, 'oht_plan_adaptive_w', False),
            plan_w_min=getattr(self.oht_config, 'oht_plan_w_min', 0.0),
            plan_w_max=getattr(self.oht_config, 'oht_plan_w_max', 4.0),
            plan_w_blk_ref=getattr(self.oht_config, 'oht_plan_w_blk_ref', 0.30),
            rail_conflict_weight=getattr(self.oht_config, 'oht_rail_conflict_weight', 1.0),
            rail_merge_weight=getattr(self.oht_config, 'oht_rail_merge_weight', 1.0),
            rail_null_action=getattr(self.oht_config, 'oht_rail_null_action', False),
            rail_congestion_threshold=getattr(self.oht_config, 'oht_rail_congestion_threshold', 4.0),
            rail_lookahead=getattr(self.oht_config, 'oht_rail_lookahead', 300.0),
            rail_reassign=getattr(self.oht_config, 'oht_rail_reassign', False),
            rail_conflict_cap_ratio=getattr(self.oht_config, 'oht_rail_conflict_cap_ratio', 1.0),
            rail_starve_guard_sec=getattr(self.oht_config, 'oht_rail_starve_guard_sec', 300.0),
            rail_adaptive_w=getattr(self.oht_config, 'oht_rail_adaptive_w', False),
            rail_w_min=getattr(self.oht_config, 'oht_rail_w_min', 1.0),
            rail_w_max=getattr(self.oht_config, 'oht_rail_w_max', 8.0),
            rail_idle_positioning=getattr(self.oht_config, 'oht_rail_idle_positioning', False),
            slack_defer=getattr(self.oht_config, 'oht_slack_defer', False),
            slack_defer_threshold=getattr(self.oht_config, 'oht_slack_defer_threshold', 0.0),
            jssp_data=None,
            schedule=self.machine_decision.schedule,
        )
    ...
    def run(self, enable_animation=None):
        ...
        # ★ deadlock recovery monitor: periodically detect + recover global wait-for cycles
        if getattr(self.oht_config, 'oht_deadlock_recovery', False):
            _DeadlockRecoveryMonitor(runner=self, env=self.env)

        # ★ (A) coverage positioning monitor: match the idle OHT distribution to the gantt demand density
        if getattr(self.oht_config, 'oht_coverage_positioning', False):
            _CoveragePositioningMonitor(runner=self, env=self.env)
        ...
        self.env.run(till=self.oht_config.sim_horizon)
    ...
    def _evaluate(self, wall_elapsed_s=None, run_mode=None):
        ...
        if _os.path.exists(tpath):
            ...
            # ★ tail metric: coverage targets the worst-case (tail), not the mean.
            def _pctl(xs, q):
                if not xs:
                    return 0.0
                s = sorted(xs)
        ...
        kpi = {
            ...
            "makespan_attain_h": round(makespan_attain_h, 3),      # ★ shortfall-converted makespan (24h+)
            "makespan_attain_rate_pct": round(makespan_attain_rate, 2),  # ★ makespan attainment rate (%), higher is better
            ...
            "source_wait_p90_s": round(sw_p90, 2),                  # ★ tail: coverage target
...
if __name__ == "__main__":
    ...
    #
    # Experiment/switch defaults are edited in the config file.
    #   - Simulation_Machine_Config.py : planning_algorithm, machine_dispatch_rule,
    #                                    release_batch_size, release_interval, etc.
    #   - Simulation_OHT_Config.py     : oht_dispatch_mode, oht_path_algorithm,
    #                                    congestion/predictive coefficients, n_oht, etc.
    #
    # Empty OHT dispatching switch:
    #   OHTConfig.oht_dispatch_mode =
    #       "GREEDY" | "HUNGARIAN" | "HUNGARIAN_REASSIGN" |
    #       "CONGESTION_AWARE_HEURISTIC" | "PREDICTIVE_DISPATCHING"
    # ═══════════════════════════════════════════════════════════════════
